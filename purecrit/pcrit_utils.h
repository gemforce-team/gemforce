#ifndef _PCRIT_UTILS_H
#define _PCRIT_UTILS_H

struct Gem_O {
	int grade;			//using short does NOT improve time/memory usage
	double crit;		//using float does NOT improve time/memory usage
	struct Gem_O* father;
	struct Gem_O* mother;
};
...
void gem_init(gem *p_gem, int grd, double crit)
{
	p_gem->grade=grd;
	p_gem->crit=crit;
	p_gem->father=NULL;
