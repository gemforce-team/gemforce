			...
			for (k=0; k< pool_length[j]; ++k) {
				...
				for (h=0; h< pool_length[i-1-j]; ++h) {
					...
					if (abs(delta)<=2) {     // grade difference <= 2
						...
						if      ( gem_rk411(temp) >= gem_rk411(temp_array[grd]) ) {						// rk411 check
							temp_array[grd]=temp;					// put in pool
						}
						else if ( gem_rk311(temp) >= gem_rk311(temp_array[ngrades+grd]) ) {			// rk311 check
							temp_array[ngrades+grd]=temp;			// put in pool
						}
						else if ( gem_power(temp) >= gem_power(temp_array[2*ngrades+grd]) ) {		// rk211 check
...
int main(int argc, char** argv)
{
	...
	if (filename[0]=='\0') strcpy(filename, "table_kgc413121");
