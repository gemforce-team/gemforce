...
#include "chit_utils.h"
#include "gfon.h"

void gem_print(gem *p_gem) {
	printf("Grade:\t%d\nC.hit:\t%f\nC.len:\t%f\n\n", p_gem->grade, p_gem->chainhit, chain_length(p_gem->chainhit));
}
...
int main(int argc, char** argv)
{
	...
	while ((opt=getopt(argc,argv,"hptecidquf:"))!=-1) {
		switch(opt) {
			case 'h':
				print_help("hptecidquf:");
				return 0;
	...
	file_selection(filename, "table_chainhit");
