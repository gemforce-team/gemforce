#ifndef _FRLEECH_UTILS_H
#define _FRLEECH_UTILS_H
...
struct Gem_OF {
	...
	struct Gem_OF* father;
	struct Gem_OF* mother;
};
...
{
	return gem1.leech*gem1.firerate;
}
...
void gem_combine (gem *p_gem1, gem *p_gem2, gem *p_gem_combined)
{
	...
	if (p_gem_combined->leech < p_gem1->leech) p_gem_combined->leech = p_gem1->leech;
	if (p_gem_combined->leech < p_gem2->leech) p_gem_combined->leech = p_gem2->leech;
}

void gem_init(gem *p_gem, int grd, double leech, double firerate)
{
	p_gem->grade =grd;
	p_gem->leech=leech;
...
{
	if (gem1.leech < gem2.leech) return 1;
	else if (gem1.leech == gem2.leech && gem1.firerate < gem2.firerate) return 1;
	else return 0;
}
