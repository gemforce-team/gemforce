...
#include <math.h>
#include <getopt.h>
typedef struct Gem_GF gem;
#include "frpoison_utils.h"

void gem_print(gem *p_gem) {
	printf("Grade:\t%d\nPoison:\t%f\nF.rate:\t%f\nPower:\t%f\n\n",
		p_gem->grade, p_gem->poison, p_gem->firerate, p_gem->poison*p_gem->firerate);
}

char gem_color(gem* p_gem) {
	if (p_gem->poison==0) return 'r';
	else return 'g';
}
...
void worker(int len, int output_options)
{
	...
	for (i=1; i<len; ++i) {
		...
		gems[i]=pool[i][0];						// choosing gem (firerateeria moved to more_power def)
