...
#include <getopt.h>
#include <string.h>
typedef struct Gem_SYB gem;    // the strange order is so that semikg_utils knows which gem type are we defining as "gem"
#include "semikg_utils.h"
#include "gfon.h"

void worker(int len, int output_options, char* filename)
{
	FILE* table=table_init(filename, 2);    // init killgem
	int i;
	int size;
	gem* pool[len];
	int pool_length[len];
