#ifndef _WKGA_UTILS_H
#define _WKGA_UTILS_H

double gem_amp_power(gem gem1, gemY amp1, double damage_ratio, double crit_ratio)
{
	return (gem1.damage+damage_ratio*amp1.damage)*pb_power(gem1.pbound)*(gem1.crit+crit_ratio*amp1.crit)*pb_power(gem1.pbound);
}
...
#include "wkgexact_utils.h"

inline gemP gem2gemP(gem g)
{
	return (gemP){g.grade, g.damage, g.crit, g.pbound, g.father, g.mother, 0};
}

inline gem gemP2gem(gemP g)
{
	return (gem){g.grade, g.damage, g.crit, g.pbound, g.father, g.mother};
}
...
#define KGSPEC_COMPRESSION																				\
	for (i=0;i<len;++i) {																				\
		...
		for (int l=length-1;l>=0;--l) {							/* start from large dmg */				\
			...
			if (ftree_check_after(tree, tree_length, p_gem->place, p_gem->pbound)) {					\
				ftree_add_element(tree, tree_length, p_gem->place, p_gem->pbound);						\
			}																							\
			else {																						\
				p_gem->grade=0;																			\
				broken++;																				\
