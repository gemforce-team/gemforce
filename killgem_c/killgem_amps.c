...
{
  return (gem1.damage*2.2+6*0.28*1.8*amp1.damage)*gem1.bbound*(gem1.crit*1.5+6*0.23*2.8*amp1.crit)*gem1.bbound;
}

int gem_alone_more_powerful(gem gem1, gem gem2, gemY amp2)
{
  return gem1.damage*2.2*gem1.bbound*gem1.crit*1.5*gem1.bbound > gem_amp_global_power(gem2, amp2);
}
...
void worker_amps(int len, int output_parens, int output_tree, int output_table, int output_debug, int output_info, int killgem_limit)
{
  printf("\n");
	int i;
	gem* pool[len];
 ...
 for (i=1; i<len/6; ++i) {			//amplifier computing
  ...
  for (i=1;i<len;++i) {																	// for every total value
		gem_init(gems+i,1,0,0,0);														// we init the gems 
		...
		gem_print_Y(amps+i);
		printf("Global power (rescaled):\t%f\n\n", (gem_amp_global_power(gems[i], amps[i])/(2.2*1.5)));
...
int main(int argc, char** argv)
{
	...
	else worker_amps(len, output_parens, output_tree, output_table, output_debug, output_info, killgem_limit);
	return 0;
}
